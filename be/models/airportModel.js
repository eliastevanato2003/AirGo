...
exports.getAirports = async (city, country, identificationcode) => {
    const sql = 'SELECT * FROM "Aeroporti" WHERE ("Citta" = $1 OR $1 IS NULL) AND ("Nazione" = $2 OR $2 IS NULL) AND ("CodiceIdentificativo" = $3 OR $3 IS NULL) AND "IsActive" = true';
    const result = await pool.query(sql, [city, country, identificationcode]);
    return result.rows;
}
