...
exports.updateUser = async (id, name, surname, number, dob, password) => {
    if (password) {
        const hashedpassword = await bcrypt.hash(password, 10);
        await userModel.updatePassword(id, hashedpassword);
    }
    return await userModel.updateUser(id, name, surname, number, dob);
}

exports.deleteUser = async (id) => await userModel.deleteUser(id);
