...
exports.newTicket = async (req, res, next) => {
    try {
        ...
        let { row, col, nseat } = req.body ?? {};
        if (flight == undefined || name == undefined || surname == undefined || dob == undefined || clas == undefined || chseat == undefined || price == undefined) res.status(400).json({ message: "Missing required data" });
            ...
            if (flights[0]) {
                ...
                if (row < 1 || col.length !== 1 || !col.toUpperCase().match(/[A-Z]/)) {
                    res.status(409).json({ message: "Invalid seat" });
                    return;
                }
                if (clas == "Prima" && flightstatus?.[0].PostiPc - flightstatus?.[0].PostiOccPc > 0) {
                    ...
                    seat = true;
                    if (chseat && row > flightstatus[0].RigheB || (col.toUpperCase().charCodeAt(0) - 64 > flightstatus[0].ColonneB)) {
                        res.status(409).json({ message: "Invalid seat" });
                        return;
                    }
                    ...
                    seat = true;
                    if (chseat && row > flightstatus[0].RigheE || (col.toUpperCase().charCodeAt(0) - 64 > flightstatus[0].ColonneE)) {
                        res.status(409).json({ message: "Invalid seat" });
                        return;
                    }
                }
